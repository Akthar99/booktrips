...
const ACTION_COPY: Record<Exclude<PendingAction, null>, { title: string; message: string; label: string; danger: boolean }> = {
    confirmed: {
        title: 'Confirm booking?',
        message: 'The guest will be notified and their contact details will be visible.',
...
export default function BookingsManager({
    ...
    const [view, setView] = useState<View>(filters.date && filters.date !== todayIso() ? 'day' : 'list');
    const [q, setQ] = useState(filters.q);
    const [status, setStatus] = useState(filters.status || 'all');
    const [date, setDate] = useState(filters.date || todayIso());
    const [pending, setPending] = useState<{ booking: BookingData; action: Exclude<PendingAction, null> } | null>(null);
    ...
    function load(overrides: Partial<{ q: string; status: string; date: string; view: View }> = {}) {
    ...
    function applyStatus(booking: BookingData, next: Exclude<PendingAction, null>) {
        setBusy(true);
        router.patch(
    ...
    return (
        <div>
            <div className="my-3 flex flex-wrap items-center gap-2.5">
                    ...
                    {(['list', 'day'] as View[]).map((option) => (
                        <button
                            ...
                            className={
                                ...
                                (view === option ? 'bg-brand-800 text-white' : 'bg-transparent text-ink')
                            }
                            onClick={() => {
                                setView(option);
                                load({ view: option });
                ...
                >
                    <Input
                        ...
                        placeholder={showBusiness ? 'Search code, guest, package, business' : 'Search code, guest, package'}
                        value={q}
            ...
            <p className="mt-2 mb-3.5 text-[13px] text-muted">
                {bookings.total} shown · {totals.requested} requested · {totals.confirmed} confirmed
            </p>

            {view === 'day' ? (
                <div className="grid gap-2.5">
                    {rows.length === 0 ? <p className="text-muted">No bookings on {date}.</p> : null}
                    {rows.map((booking) => (
                        <div key={booking.id} className="rounded-[14px] border border-line bg-white px-4 py-3.5">
                            <strong>
                                {booking.booking_code} · {booking.package?.title}
                            </strong>
                            <div className="text-[13px] text-muted">
                                {showBusiness && booking.host?.name ? <>{booking.host.name} · </> : null}
                                {booking.guest_name} · {booking.guests} guests · {lkr(booking.total_lkr)}
                            </div>
                            <div className="mt-2 flex flex-wrap items-center gap-2">
                                ...
                                <Actions
                                    booking={booking}
                                    mode={mode}
                                    onAction={(action) => setPending({ booking, action })}
                                />
                            </div>
                        </div>
                    ))}
                    ...
                    <table className="w-full border-collapse text-left">
                        <thead>
                            <tr className="bg-cream-dark text-[11px] tracking-wide text-muted uppercase">
                                ...
                                {showBusiness ? <th className="px-3.5 py-3 font-bold">Business</th> : null}
                        ...
                        <tbody>
                            {rows.map((booking) => (
                                <tr key={booking.id} className="border-t border-line align-middle">
                                    <td className="px-3.5 py-3 text-sm whitespace-nowrap">{booking.booking_code}</td>
                                    <td className="px-3.5 py-3 text-sm">{booking.package?.title}</td>
                                    {showBusiness ? (
                                        <td className="px-3.5 py-3 text-sm">
                                            ...
                                            {booking.host?.email ? (
                                                <>
                                                    <br />
                                                    <span className="text-muted">{booking.host.email}</span>
                                                </>
                                            ) : null}
                                        </td>
                                    ) : null}
                                    ...
                                    <td className="px-3.5 py-3 text-sm">
                                        {mode === 'admin' ? (
                                            ...
                                            <Link className="font-bold text-brand-800" href={`/partners/bookings/${booking.id}`}>
                                                {booking.guest_name}
                                            </Link>
                                        )}
                                        <br />
                                        <span className="text-muted">
                                            {booking.contact_hidden ? 'Contact after confirm' : booking.guest_phone || '—'}
                                        </span>
                                    </td>
                                    <td className="px-3.5 py-3 text-sm whitespace-nowrap">{lkr(booking.total_lkr)}</td>
            ...
            <ConfirmDialog
                ...
                busy={busy}
                onConfirm={() => pending && applyStatus(pending.booking, pending.action)}
    ...
    if (booking.status === 'requested') {
        return (
            <span className="inline-flex items-center gap-3.5 whitespace-nowrap">
                <button
                    type="button"
                    className="cursor-pointer border-0 bg-transparent p-0 text-[13px] font-bold text-brand-800 hover:underline"
                ...
                <button
                    type="button"
                    className="cursor-pointer border-0 bg-transparent p-0 text-[13px] font-bold text-danger hover:underline"
