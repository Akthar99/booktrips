    ...
    useEffect(() => {
        ...
        async function boot() {
            ...
            const leaflet = (await import('leaflet'))
                .default as unknown as LeafletModule;
            ...
            const map = leaflet
                .map(container.current, { scrollWheelZoom: interactive })
                .setView(start, zoom);
        ...
        function draw() {
            ...
            pins.filter((pin) => pin.lat && pin.lng).forEach((pin) => {
                const title = escapeHtml(
                    pin.title || pin.location || 'Package',
                );
                const location = escapeHtml(pin.location || '');
                const initial = escapeHtml(
                    (pin.title || pin.location || 'P')
                        .trim()
                        .slice(0, 1)
                        .toUpperCase(),
                );
                ...
                if (pin.kind === 'you') {
                    ...
                    if (pin.title) {
                        const link = pin.id
                            ? `<a href="/packages/${pin.id}">${title}</a>`
                            : title;
                        marker.bindPopup(
                            `${link}${location ? `<br/>${location}` : ''}`,
                        );
                    }
                }
