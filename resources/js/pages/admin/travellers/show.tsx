        ...
        <div className="mx-auto w-[min(1180px,calc(100%-2rem))] py-7 pb-14">
            ...
            <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
                <div>
                    ...
                    <p className="text-muted">
                        {traveller.email}
                        {traveller.phone ? ` · ${traveller.phone}` : ''} · joined{' '}
                        {traveller.created_at
                            ? new Date(traveller.created_at).toLocaleDateString('en-GB')
                            : '—'}
                    </p>
                </div>
                <div className="flex flex-col items-end gap-1">
                    <StatusBadge status={traveller.active ? 'active' : 'suspended'} />
                    {traveller.strikes > 0 ? (
                        <span className="rounded-full bg-orange-50 px-2.5 py-1 text-[12px] font-bold text-warn">
                            {traveller.strikes} strike{traveller.strikes === 1 ? '' : 's'}
                        </span>
                    ) : null}
            ...
            <div className="grid gap-4 lg:grid-cols-2">
                <section className="rounded-2xl border border-line bg-white p-5">
                    ...
                    {disputes.length === 0 ? (
                        <p className="text-[13px] text-muted">No reports involving this traveller.</p>
                    ) : (
                            ...
                            <div key={dispute.id} className="mb-2 rounded-xl border border-line px-3 py-2.5">
                                <div className="flex items-center justify-between gap-2 text-[13px]">
                                    ...
                                    <span className="text-muted">
                                        {dispute.raised_by_me ? 'raised by them' : 'against them'}
                                    </span>
                                </div>
                                <p className="text-[13px]">{dispute.summary}</p>
                                <p className="text-[12px] text-muted">
                                    {dispute.business_name} · {dispute.booking_code}
