...
const VerifyEmail: InertiaComponent<VerifyProps> = ({
    email,
    pendingEmail,
}) => {
            ...
            <div className="border-line shadow-card w-full max-w-[440px] rounded-[20px] border bg-white p-7 text-center">
                ...
                {isVerified ? (
                    <p className="text-muted mb-5">
                        {email || user?.email} is verified. You can book trips
                        now.
                    </p>
                ) : (
                    <p className="text-muted mb-5">
                        We sent a verification link to{' '}
                        <strong>{email || user?.email}</strong>. Open it to
                        unlock booking. Booking needs a verified email, so
                        request a new link if it went missing.
                    </p>
                )}
                {pendingEmail ? (
                    <Alert tone="note">
                        Waiting on confirmation for{' '}
                        <strong>{pendingEmail}</strong>. Open the link we sent
                ...
                {isVerified ? (
                    ...
                    <button
                        ...
                        className="bg-brand-800 hover:bg-brand-900 cursor-pointer rounded-full px-4.5 py-2.5 text-sm font-bold text-white transition disabled:cursor-default disabled:opacity-60"
                        onClick={() => {
                            resend.start(60);
                            router.post(
