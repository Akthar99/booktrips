...
const Login: InertiaComponent<LoginProps> = ({ status }) => {
    ...
    return (
        <div className="flex min-h-[calc(100vh-4.5rem)] items-center justify-center px-4 pt-8 pb-16">
            <Form action="/login" method="post" className="w-full max-w-[440px] rounded-[20px] border border-line bg-white p-7 shadow-card">
                {({ errors, processing }) => (
                    <>
                        <h1 className="mb-2 text-3xl">Log in</h1>
                        <p className="mb-5 text-muted">
                            Reserve trips after your email is verified. Pay at the destination.
                        </p>
                        {page.props.flash?.error ? <Alert tone="error">{page.props.flash.error}</Alert> : null}
                        {status ? <Alert tone="success">{status}</Alert> : null}
                        {errors.email ? <Alert tone="error">{errors.email}</Alert> : null}
                        <Field label="Email">
                        ...
                        <p className="mt-3.5 flex gap-4 text-sm">
                            <Link className="font-bold text-brand-800" href="/forgot-password">
                                Forgot password
                            </Link>
                        </p>
                        <p className="mt-4 text-sm">
                            No account?{' '}
                            <Link className="font-bold text-brand-800" href="/register">
                                Sign up
                            </Link>
                        </p>
                    </>
