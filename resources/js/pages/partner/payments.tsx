...
function ReceiptForm({ invoice }: { invoice: InvoiceData }) {
    const form = useForm<{ invoice_id: number; note: string; file: File | null }>({
    ...
    return (
        <form onSubmit={submit} className="mt-3">
            {Object.values(form.errors)[0] ? <Alert tone="error">{Object.values(form.errors)[0]}</Alert> : null}
...
const Payments: InertiaComponent<PaymentsProps> = ({ bank, commissionRate, invoices }) => {
    const percent = Math.round(commissionRate * 100);
    const [expanded, setExpanded] = useState<number | null>(invoices.find((invoice) => invoice.status !== 'paid')?.id ?? null);
    ...
    return (
        <div className="mx-auto w-[min(1180px,calc(100%-2rem))] py-7 pb-14">
            ...
            <p className="mb-4 text-muted">
                You do not pay per booking. Finished stays add {percent}% to one bill dated the last day of the
                month. Transfer that total once and upload the receipt.
            </p>

            <div className="mb-5 rounded-[14px] border border-line bg-white px-4 py-3.5">
            ...
            {invoices.length === 0 ? (
                <p className="text-muted">No bills yet. They appear after you mark a booking finished.</p>
            ) : null}

            {invoices.map((invoice) => (
                <div key={invoice.id} className="mb-3 rounded-[14px] border border-line bg-white px-4 py-3.5">
                    <div className="flex flex-wrap justify-between gap-3">
                        <div>
                            <strong>{invoice.period_label}</strong>
                            <div className="text-[13px] text-muted">
                                Due {invoice.due_date} · {(invoice.lines ?? []).length} finished bookings
                                {invoice.paid_at ? ` · paid ${formatDateTime(invoice.paid_at)}` : ''}
                            </div>
                        </div>
                        <div className="text-right">
                            <div className="font-extrabold text-brand-900">{lkr(invoice.amount_lkr)}</div>
                    ...
                    {invoice.status !== 'paid' ? (
                        <>
                            <button
                                type="button"
                                className="mt-2.5 cursor-pointer border-0 bg-transparent p-0 text-[13px] font-bold text-brand-800"
                                onClick={() => setExpanded(expanded === invoice.id ? null : invoice.id)}
                            >
                                {expanded === invoice.id ? 'Hide receipt upload' : 'Upload a receipt'}
                            </button>
                            {expanded === invoice.id ? <ReceiptForm invoice={invoice} /> : null}
                        </>
                    ) : null}

                    {invoice.receipts.map((receipt) => (
                        <p key={receipt.id} className="mt-2 text-[13px]">
                            Receipt {receipt.status} · {formatDateTime(receipt.created_at)} ·{' '}
                            <a
