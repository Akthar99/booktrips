...
const Home: InertiaComponent<HomeProps> = ({
    featured,
    pins,
    categories,
    destinations,
        ...
        <>
            <section className="relative flex min-h-[540px] items-end py-10 pb-14 text-white md:min-h-[620px]">
                <div
                    ...
                    style={{
                        backgroundImage:
                            "url('/Caymass%20Holidays%20Pvt%20Ldt.png')",
                    }}
                />
                ...
                <div className="relative z-10 mx-auto w-[min(1180px,calc(100%-2rem))]">
                    ...
                    <p className="mb-7 max-w-[46ch] text-[17px] text-[#e8efe9]">
                        Camping, villas, hotel packages and activity days from
                        operators across the island. You reserve here. You pay
                        when you arrive.
                    </p>
                    <SearchBar
                ...
                <div className="mx-auto w-[min(1180px,calc(100%-2rem))]">
                    <div className="mb-6 flex items-end justify-between gap-4">
                        <div>
                            ...
                            <p className="text-muted">
                                Same idea as a hotel search — but for trips, not
                                only rooms.
                            </p>
                        </div>
                    </div>
                    <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-6">
                        {categories.map((category) => {
                            const Icon =
                                CATEGORY_ICONS[category.icon] ?? Compass;

                            return (
                                <Link
                                    key={category.slug}
                                    href={`/search?category=${category.slug}`}
                                    className="border-line hover:border-brand-500 hover:shadow-card flex min-h-[118px] flex-col gap-2 rounded-2xl border bg-white px-3.5 py-4 transition hover:-translate-y-0.5"
                                >
                                    <span className="bg-brand-50 text-brand-800 grid h-9.5 w-9.5 place-items-center rounded-[10px]">
                                        <Icon size={18} />
                                    </span>
                                    <strong className="text-sm">
                                        {category.name}
                                    </strong>
                                    <em className="text-muted text-xs font-normal not-italic">
                                        {category.blurb}
                                    </em>
                                </Link>
                            );
                ...
                <div className="mx-auto w-[min(1180px,calc(100%-2rem))]">
                    <div className="mb-6 flex items-end justify-between gap-4">
                        <div>
                            ...
                            <p className="text-muted">
                                Hand-picked packages with strong reviews.
                            </p>
                        </div>
                        <Link
                            href="/search"
                            className="text-brand-800 text-sm font-bold"
                        >
                            See all packages
                        </Link>
                    </div>
                ...
                <div className="mx-auto w-[min(1180px,calc(100%-2rem))]">
                    <div className="mb-6 flex items-end justify-between gap-4">
                        <div>
                            ...
                            <p className="text-muted">
                                Every live package on the map. Open it to sort
                                by distance.
                            </p>
                        </div>
                        <Link
            ...
            <section className="pb-14">
                <div className="mx-auto w-[min(1180px,calc(100%-2rem))]">
                    <h2 className="mb-6 text-3xl">
                        Where Sri Lanka goes on weekends
                    </h2>
            ...
            <section className="bg-navy-900 py-16 text-[#e8efe9]">
                <div className="mx-auto w-[min(1180px,calc(100%-2rem))]">
                    <h2 className="mb-2.5 text-4xl text-white">
                        How BookTrips works
                    </h2>
                    <p className="max-w-[50ch] text-[#b7c4d1]">
                        No card on file. A verified email is enough to hold the
                        date.
                    </p>
                    <div className="mt-7 grid gap-4.5 md:grid-cols-3">
                        {[
                            [
                                '01',
                                'Find a package',
                                'Filter by camping, day out, villa, hotel or activity — then pick dates.',
                            ],
                            [
                                '02',
                                'Verify & reserve',
                                'Log in, confirm your email, and the host sees your booking code.',
                            ],
                            [
                                '03',
                                'Pay at destination',
                                'Settle in rupees with the hotel, villa or guide when you arrive.',
                            ],
                        ].map(([step, title, copy]) => (
                            <div
                                key={step}
                ...
                <div className="mx-auto grid w-[min(1180px,calc(100%-2rem))] items-center gap-7 lg:grid-cols-[1.2fr_1fr]">
                    <div>
                        <h2 className="text-3xl">
                            Hotels, villas and operators
                        </h2>
                        <p className="text-muted mt-3 mb-5 max-w-[520px]">
                            If you already host guests in Ella, run jeeps in
                            Yala, or cook camp meals in the Knuckles, list a
                            package. Travellers book it. They pay you on site.
                        </p>
                        <Link
                            href="/partners"
                            className="bg-navy-900 hover:bg-navy-950 inline-flex rounded-full px-4.5 py-2.5 text-sm font-bold text-white transition"
                        >
                            List your package
                        </Link>
                    </div>
