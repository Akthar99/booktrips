...
const Apply: InertiaComponent<ApplyProps> = ({
    businessTypes,
    mode,
    account,
    verifiedPhone,
    ...
    const socialsError = (form.errors as Record<string, string | undefined>)
        .socials;
    ...
    function submit(event: React.FormEvent) {
        ...
        if (!verified) {
            setVerifyNotice(
                'Verify your mobile number before submitting the application.',
            );
    ...
    return (
        <div className="mx-auto w-[min(720px,calc(100%-2rem))] py-7 pb-14">
            ...
            <h1 className="mt-2 text-4xl">
                {upgrading ? 'Become a partner' : 'Request to join'}
            </h1>
            ...
            {upgrading ? (
                <Alert tone="note">
                    You are signed in as {account?.email}. Submitting upgrades
                    this account, so there is no new password to choose and your
                    past bookings stay with you.
                </Alert>
            ) : null}
            ...
            >
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                    ...
                    <Field label="Phone">
                        ...
                        />
                        {form.errors.phone ? (
                            <Alert tone="error">{form.errors.phone}</Alert>
                        ) : null}
                    </Field>
                </div>
                ...
                />
                {verifyNotice ? (
                    <Alert tone="error">{verifyNotice}</Alert>
                ) : null}
                ...
                <h3 className="mt-4 mb-3 font-sans text-lg font-bold">
                    Business
                </h3>
                <Field label="Business name">
                    ...
                    {form.errors.business_name ? (
                        <Alert tone="error">{form.errors.business_name}</Alert>
                    ) : null}
                </Field>
                <Field label="Type">
                    <Select
                ...
                <Field label="About the business">
                    ...
                    {form.errors.description ? (
                        <Alert tone="error">{form.errors.description}</Alert>
                    ) : null}
                </Field>
                <div className="grid gap-3 sm:grid-cols-2">
                    <Field label="City">
                        ...
                        />
                        {form.errors.city ? (
                            <Alert tone="error">{form.errors.city}</Alert>
                        ) : null}
                    </Field>
                ...
                <p className="text-muted -mt-2 mb-3 text-[13px]">
                    Add at least one of these — we check it before approving
                    your application.
                </p>
                {socialsError ? (
                    <Alert tone="error">{socialsError}</Alert>
                ) : null}
                <Field label="Website">
                    <Input
                        placeholder="https://"
                ...
                <Field label="Instagram">
                    <Input
                        ...
                        onChange={(event) =>
                            form.setData('instagram', event.target.value)
                        }
                    />
                </Field>
                <div className="grid gap-3 sm:grid-cols-2">
                    <Field label="Facebook">
                        <Input
                            ...
                            onChange={(event) =>
                                form.setData('facebook', event.target.value)
                            }
                        />
                    </Field>
                    <Field label="TikTok">
                        <Input
                            ...
                            onChange={(event) =>
                                form.setData('tiktok', event.target.value)
                            }
                        />
                    </Field>
                </div>
