...
const BookingShow: InertiaComponent<BookingShowProps> = ({
    booking,
    disputes,
    reportTypes,
    canReport,
    ...
    return (
        <div className="mx-auto w-[min(1180px,calc(100%-2rem))] py-12">
            <div className="border-line mx-auto max-w-[640px] rounded-[20px] border bg-white p-8 text-center text-left">
                <h1 className="mb-3 text-center text-3xl">
                    Booking {booking.booking_code}
                </h1>
                <div className="text-brand-800 mb-3 text-center text-[28px] font-extrabold tracking-[0.08em]">
                    {booking.booking_code}
                </div>
                <p className="text-center">{booking.package?.title}</p>
                <p className="text-muted text-center">
                    {booking.package?.location} · {booking.check_in} →{' '}
                    {booking.check_out} · {booking.guests} guests
                </p>
                ...
                <p className="my-2 mb-5 text-center">
                    ...
                    {booking.escalated && booking.status === 'requested' ? (
                        <span className="text-warn ml-2 text-[13px]">
                            Waiting for the host for over 24h
                        </span>
                    ) : null}
                </p>
                {booking.host ? (
                ...
                {booking.notes ? (
                    <p className="text-muted mt-3 text-center text-[13px]">
                        Your note: {booking.notes}
                    </p>
                ) : null}

                {booking.status === 'completed' ? (
                    <div className="border-brand-100 bg-brand-50 mx-auto mt-5 rounded-[14px] border p-4.5">
                        {booking.review ? (
                            <>
                            ...
                            <form onSubmit={submitReview}>
                                ...
                                <p className="text-muted my-1.5 mb-3 text-[13px]">
                                    Your rating helps other travellers choose
                                    well.
                                </p>
