...
const Book: InertiaComponent<BookProps> = ({ package: pkg, verifiedPhone }) => {
    ...
    function submit(event: React.FormEvent) {
        ...
        if (!phoneVerified) {
            setError(
                'Verify your mobile number with the code we text you before sending the request.',
            );
    ...
    if (!user || !user.email_verified) {
        return (
            <div className="mx-auto w-[min(1180px,calc(100%-2rem))] py-12">
                <div className="border-line mx-auto max-w-[640px] rounded-[20px] border bg-white p-8 text-center">
                    <h1 className="mb-2 text-3xl">Verify email to book</h1>
                    <p className="text-muted mb-5">
                        We only confirm trips for verified addresses so hosts
                        can trust the reservation.
                    </p>
                    <Link
                        href="/verify-email"
    ...
    return (
        <div className="mx-auto grid w-[min(1180px,calc(100%-2rem))] grid-cols-1 gap-6 py-8 pb-16 lg:grid-cols-[1fr_320px]">
            ...
            >
                ...
                <Field label="Lead guest name">
                    <Input
                        ...
                        onChange={(event) =>
                            setForm({ ...form, guest_name: event.target.value })
                        }
                    />
                </Field>
                <Field label="Phone">
                ...
                <PhoneVerification
                    ...
                    onPhoneChange={(value) =>
                        setForm({ ...form, guest_phone: value })
                    }
                ...
                <div className="bg-brand-50 text-brand-950 my-2 mb-3.5 rounded-xl px-3 py-2.5 text-[13px]">
                    Payment method: <strong>Pay at destination</strong>. No card
                    is charged on BookTrips.
                </div>
                <button
                    type="submit"
            ...
            <aside className="rounded-card border-line shadow-card h-fit border bg-white p-4.5 lg:sticky lg:top-22">
                {pkg.images?.[0] ? (
                    <img
                        src={pkg.images[0]}
                        alt=""
                        className="mb-3 h-35 w-full rounded-xl object-cover"
                    />
                ) : null}
                <h3 className="font-sans text-base font-bold">{pkg.title}</h3>
                <p className="text-muted text-[13px]">{pkg.location}</p>
                {quote ? (
                    <div className="text-brand-900 mt-3 text-base font-extrabold">
                        {quote.discount_lkr ? (
                            <del className="text-muted block text-xs font-semibold">
                                {lkr(quote.base_total_lkr)}
                            </del>
                        ) : null}
                        {lkr(quote.total_lkr)}
